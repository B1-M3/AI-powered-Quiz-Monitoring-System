...
<div id="apexia-quiz-steps-wrapper" class="container-fluid position-relative" style="z-index: 1;">
    ...
    <div class="card shadow-sm p-4 mb-4">
        ...
        <p class="text-muted small mb-0"><strong>5 questions</strong> (chosen at random from a pool of <strong>50</strong> – each student gets a different set). <strong><?php echo e(($quiz->questions && $quiz->questions->isNotEmpty() ? optional($quiz->questions->first())->time_limit_minutes : null) ?? 3); ?> minutes</strong> per question. Session is recorded; after submission the recording is saved under <strong>Lecture Risk Analysis</strong> with an accuracy level.</p>
        <?php else: ?>
        <p class="text-muted small mb-0"><strong><?php echo e($quiz->questions_per_attempt ?? 5); ?> questions</strong> (chosen at random from the pool – each student gets a different set). <strong><?php echo e(($quiz->questions && $quiz->questions->isNotEmpty() ? optional($quiz->questions->first())->time_limit_minutes : null) ?? 3); ?> minutes</strong> per question.</p>
        <?php endif; ?>
    </div>
    ...
    <div id="apexia-step0" class="card shadow-sm p-4 mb-3 border-success">
        ...
        <p class="small text-muted mb-3">Click the green button below to start. You will answer <strong><?php echo e($quiz->questions_per_attempt ?? 5); ?> random questions</strong> with a time limit per question.</p>
        <?php endif; ?>
    ...
    <div id="apexia-step4" class="card shadow-sm p-4 mb-3" style="display:none;">
        ...
        <?php endif; ?>
        <p class="mb-2">Time limit: <strong><?php echo e($quiz->time_limit_minutes); ?></strong> min. Attempts: <strong><?php echo e($quiz->attempts_allowed ?? 2); ?></strong>.</p>
...
<div id="apexia-quiz-in-progress" class="apexia-quiz-fullscreen position-fixed top-0 start-0 w-100 h-100 d-flex flex-column overflow-hidden <?php echo e($hideCameraAndRecording ? 'apexia-no-camera bg-light' : 'apexia-quiz-bg'); ?>" style="display:none; z-index: 9998;">
    ...
    <div class="apexia-quiz-topbar d-flex justify-content-between align-items-center flex-wrap gap-2 px-3 py-2 bg-dark bg-opacity-85 text-white border-bottom border-secondary position-relative z-2">
        ...
        <span class="d-flex align-items-center gap-2" id="apexia-recording-label">
            ...
            <span id="apexia-session-info" class="small opacity-90">5 questions · 3 min each · 15 min total</span>
        </span>
        <?php endif; ?>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p class="small mb-2"><strong><?php echo e($quiz->questions_per_attempt ?? 5); ?> questions</strong> (chosen at random from the pool of <?php echo e($quiz->questions ? $quiz->questions->count() : 50); ?> – each student gets a different set). <strong><?php echo e(($quiz->questions && $quiz->questions->isNotEmpty() ? optional($quiz->questions->first())->time_limit_minutes : null) ?? 3); ?> minutes</strong> per question · <strong><?php echo e(($quiz->questions_per_attempt ?? 5) * (($quiz->questions && $quiz->questions->isNotEmpty() ? optional($quiz->questions->first())->time_limit_minutes : 3) ?? 3)); ?> minutes</strong> total.</p>
                <?php if(empty($hideCameraAndRecording)): ?>
...
<?php echo $__env->make('inc.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\RPTP\Desktop\Apexia Academic Management System\resources\views/student/quiz_take.blade.php ENDPATH**/ ?>